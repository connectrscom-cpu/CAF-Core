...
/**
 * Project-scoped prompt versions: fork Flow Engine templates without editing global `prompt_templates`.
 * Saves via `/api/project-config/project-prompts` → Core `POST /v1/projects/:slug/prompt-versions`.
 */
export function ProjectPromptsPanel() {
  ...
  }, [ready, multiProject, activeProjectSlug]);
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
        <div className="project-prompts-col" style={{ minWidth: 320, flex: "1 1 320px" }}>
          ...
          <p style={{ margin: "0 0 12px", color: "var(--muted)", fontSize: 13, lineHeight: 1.45 }}>
            Read-only preview of each Flow Engine template below. Use <strong>+ New version</strong> to copy text into
            this project&apos;s <span className="mono">caf_core.prompt_versions</span> rows — generation merges those
            overrides on top of the global template. Set one row <strong>active</strong> per flow (or use <strong>test</strong> for
            experiments). CAF-wide template edits stay in Core <em>Prompt labs</em> / Flow Engine.
          </p>
          ...
          {currentTemplate ? (
            <>
              ...
              <details style={{ marginBottom: 12 }}>
                <summary style={{ cursor: "pointer", color: "var(--muted)" }}>Global template preview (read-only)</summary>
