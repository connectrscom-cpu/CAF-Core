...
export async function jsonTaskDetailResponse(
  ...
  try {
    ...
    const data: Record<string, string | undefined> = {
      ...
      is_mimic_replication: job.is_mimic_replication ? "true" : "false",
      preview_url,
      video_url,
      ...
      text_overlay_reprint_status: reprintState.status ?? undefined,
...
async function lookupQueueRowByTaskId(
  ...
  for (const tab of tabs) {
    ...
    return {
      ...
      flow_label: match.flow_label ?? match.flow_type ?? "",
      flow_detail: match.flow_detail ?? "",
      is_mimic_replication: match.is_mimic_replication ? "true" : "false",
      preview_url,
