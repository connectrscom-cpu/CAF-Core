...
describe("normalizeMimicPickRef", () => {
  it("strips __why_mimic suffix and upgrades kind to why_carousel", () => {
    expect(normalizeMimicPickRef("ins_7d15863d9f_28240_cdeep__why_mimic", "carousel")).toEqual({
      insights_id: "ins_7d15863d9f_28240_cdeep",
...
describe("buildSignalPackMimicReferencesForUi", () => {
  it("groups visual guideline entries by mimic kind", () => {
        ...
        [SIGNAL_PACK_DERIVED_GLOBALS_KEYS.visualGuidelinesPackV1]: {
          entries: [
            ...
            },
            {
              ...
              evidence_kind: "instagram_post",
              evidence_post_url: "https://www.instagram.com/p/ABC123/",
    ...
    expect(rows.map((r) => r.mimic_kind).sort()).toEqual(["carousel", "image", "why_carousel"]);
    ...
    expect(grouped.get("mimic_why_carousel")).toHaveLength(1);
    ...
    expect(carouselRow?.evidence_post_url).toBe("https://www.instagram.com/p/ABC123/");
  });
...
describe("plannerRowsFromMimicPicks", () => {
  ...
  it("creates FLOW_WHY_MIMIC_CAROUSEL rows for why_carousel picks", () => {
    const pack = {
      derived_globals_json: {
        [SIGNAL_PACK_DERIVED_GLOBALS_KEYS.visualGuidelinesPackV1]: {
          entries: [
    ...
    const rows = plannerRowsFromMimicPicks(
      pack,
      [{ insights_id: "ins_car_1", mimic_kind: "why_carousel" }],
      "RUN_TEST"
    );
    expect(rows).toHaveLength(1);
    expect(rows[0]?.target_flow_type).toBe(mimicKindToFlowType("why_carousel"));
    expect(rows[0]?.mimic_kind).toBe("why_carousel");
    expect(rows[0]?.grounding_insight_ids).toEqual(["ins_car_1"]);
  });

  it("normalizes legacy __why_mimic pick_id suffix on materialize", () => {
    const pack = {
      derived_globals_json: {
        [SIGNAL_PACK_DERIVED_GLOBALS_KEYS.visualGuidelinesPackV1]: {
          entries: [
            {
              insights_id: "ins_7d15863d9f_28240_cdeep",
              analysis_tier: "top_performer_carousel",
              source_evidence_row_id: "28240",
              evidence_kind: "instagram_post",
              hook_text_preview: "Why deck",
            },
          ],
        },
      },
    ...
    const rows = plannerRowsFromMimicPicks(
      pack,
      [{ insights_id: "ins_7d15863d9f_28240_cdeep__why_mimic", mimic_kind: "carousel" }],
      "RUN_TEST"
    );
    expect(rows).toHaveLength(1);
    expect(rows[0]?.target_flow_type).toBe(mimicKindToFlowType("why_carousel"));
    expect(rows[0]?.grounding_insight_ids).toEqual(["ins_7d15863d9f_28240_cdeep"]);
  });
