...
- The aggregate may include **edit_evidence**: before→after diffs of what human operators actually rewrote (title/hook/caption/hashtags/spoken_script/slides) plus **reprint** counts (mimic text-overlay recomposites: typography, layer_positions, slide_copy, text_backing, logo/frame overlays). Edit diffs are the strongest ground truth about the desired output — infer style/copy guidelines from the direction of the edits (e.g. operators consistently shorten hooks, remove emojis, or anglicize CTAs), and treat repeated reprint adjustments as template/layout defects, not copy problems.
