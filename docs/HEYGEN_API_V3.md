...
- **API audit (`api_call_audit`, step `heygen_video_generate`):** `requestJson` stores both `request_body_pre_normalization` (CAF-internal shape, may include `duration_sec` for debugging) and `request_body_sent` (payload after `normalizeHeyGenSubmitPayload`, i.e. what was actually stringified for `fetch`). This makes it obvious when a field appears in audit but not on the wire.
