/**
 * Sample the slide background under each Document AI text layer and pick readable fg + shadow.
 * Runs inside Puppeteer via page.evaluate (must stay self-contained — no outer closures).
 */

/** @param {import('puppeteer').Page} page */
async function adaptMimicDocAiTextContrast(page) {
  await page.evaluate(adaptMimicDocAiTextContrastInPage);
}

async function adaptMimicDocAiTextContrastInPage() {
  const CANVAS_W = 1080;
  const CANVAS_H = 1350;
  ...
  function contrastStyleForLuminance(lum) {
  ...
  function parseBackgroundImageUrl(bgImageCss) {
  ...
  function avgLuminanceInRect(ctx, x, y, w, h) {
  ...
  async function buildBackgroundSampler(slideEl) {
    const bgEl = slideEl.querySelector(".slide-bg");
    const url =
      bgEl && parseBackgroundImageUrl(bgEl.style.backgroundImage || getComputedStyle(bgEl).backgroundImage);
    ...
    const slideBg = parseCssColor(getComputedStyle(slideEl).backgroundColor);
    const pageBg = parseCssColor(getComputedStyle(slideEl.querySelector(".page") || slideEl).backgroundColor);
  ...
  for (const slideEl of slides) {
    ...
    for (const el of layers) {
      if (el.classList.contains("mimic-docai-layer--text-back")) {
...
module.exports = {
  adaptMimicDocAiTextContrast,
  adaptMimicDocAiTextContrastInPage,
};
